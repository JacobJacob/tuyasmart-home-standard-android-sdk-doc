 ...
 */
public class StandardCommonDebugDeviceAdapter extends BaseListArrayAdapter<StandardSchemaBean> {
    ...
    public StandardCommonDebugDeviceAdapter(Context context, DeviceBean bean, int resource, List<StandardSchemaBean> data) {
        super(context, resource, data);
        mBean = bean;
        mViewList = new ArrayList<>(20);
    }

    public StandardCommonDebugDeviceAdapter(Context context, SchemaBean schemaBean, DeviceBean bean, int resource, List<StandardSchemaBean> data) {
        super(context, resource, data);
        mBean = bean;
        mViewList = new ArrayList<>(20);
    ...
    @Override
    protected void bindView(ViewHolder viewHolder, StandardSchemaBean data) {
    ...
    private StandardSchemaBean getSchemaBeanById(String schemaId) {
//        List<StandardSchemaBean> data = getData();
//        for (StandardSchemaBean schemaBean : data) {
...
//        }
    ...
    private static class InnerViewHolder extends ViewHolder<StandardSchemaBean> {
        ...
        @Override
        public void initData(StandardSchemaBean schemaBean) {
            dpName.setText(schemaBean.getDpCode());
            if (schemaBean.getValue() instanceof Integer) {
                dpNumber.setText(schemaBean.getValue().toString());
            }
        ...
        private void initListener(StandardSchemaBean schemaBean) {
            contentView.setTag(R.id.schemaId, schemaBean.getDpCode());
            String type = schemaBean.getStandardType();
            if ("Boolean".equalsIgnoreCase(type) || "bool".equalsIgnoreCase(type)) {
                ...
                sbBoolean.setTag(R.id.schemaId, schemaBean.getDpCode());
            } else if ("Integer".equals(type) || "value".equalsIgnoreCase(type)) {
...
//            }
        ...
        private void initDpData(StandardSchemaBean schemaBean, DeviceBean bean) {
            dpName.setText(schemaBean.getDpCode());
            ...
            if ("Boolean".equals(type) || "bool".equals(type)) {
                ...
                try {
                    sbBoolean.setCheckedImmediatelyNoEvent((Boolean) schemaBean.getValue());
                ...
                try {
                    numberValue.setText(schemaBean.getValue() + "");
                } catch (Exception ignored) {
                    ignored.printStackTrace();
                }
            } else if ("Enum".equalsIgnoreCase(type)) {
                ...
                try {
                    mEnumTV.setText((String) schemaBean.getValue());
                } catch (Exception ignored) {
                    ignored.printStackTrace();
...
//            }
        ...
        private static String getProperty(StandardSchemaBean schemaBean) {

            String schemaTypeStr = "";
//            if (schemaBean.getStandardType().equals(DataTypeEnum.OBJ.getType())) {
            //obj 类型
            String schemaType = schemaBean.getStandardType();
//                if (TextUtils.equals(schemaType, StringSchemaBean.type)) {
